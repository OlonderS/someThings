...
namespace cwiczenia
{
    ...
    public class Zespol : ICloneable
    {
        ...
        {
            BinaryFormatter bf = new BinaryFormatter(); //XmlSerializer (typeof())
            using (FileStream FileShare = new FileStream(fname, FileMode.Create)) //StreamWriter, samo fname
            {
        ...
        public static Zespol OdczytajBin(string fname)
        {
            ...
            if (!File.Exists(fname))
                return null;
            using (FileStream fs = new FileStream(fname, FileMode.Open)) //StreamReader, samo fname
            {
                return bf.Deserialize(fs) as Zespol;
            }
        }

        public static Zespol OdcztytajXml(string nazwa)
        {
            ...
            {
                return (Zespol)serializer.Deserialize(sr);
            }
